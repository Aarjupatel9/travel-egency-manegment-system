...
//         //table present
//         if ($table_isornot->num_rows > 0) {
//             //insert ticket reservation info into table
//             for($i= $table_isornot->num_rows; i<($table_isornot->num_rows+))
//             $add_ticket_info_for_bus = "INSERT INTO $table_name(`sit_number`, `user_email`, `departure_time`) VALUES ('','[value-2]','[value-3]')";
//             $ticket_info = $conn->query($Select_ticket_info_for_bus);

//             if ($ticket_info->num_rows > 0) {
//                 //echo "enter in if";
//                 $counter_ticket = 0;
//                 while ($row = $ticket_info->fetch_assoc()) {

//                     $ticket_data[$counter_ticket] = array(
//                         "bus_number" => $row["bus_number"],
//                         "destination" => $row["destination"],
//                         "source" => $row["source"],
//                         "capacity" => $row["capacity"],
//                         "reserved_ticket" => $row["reserved_ticket"],
//                     );
//                     $counter_ticket++;
//                     $route_data[$counter]["reserved_ticket"] = $row["reserved_ticket"];
//                 }
